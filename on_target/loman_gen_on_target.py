...
if __name__ == "__main__":
    ...
    tsv_rows = [["Sample header", "MEGARes header", "Coverage", "Matches (nts)", "Matches (blks)", "Ins (nts)", "Ins (blks)", "Dels(nts)", "Dels(blks)", \
                 "CIGAR length (nts)", "Length of alignment on sample", "Length of alignment on MEGARes"]]
    ...
    if sys.argv[2] == "1":
        ...
        sam_file = pysam.AlignmentFile(sys.argv[3] + "_megares_mapped.sam", 'r')
        #Iterate through every read. Accumulate number of reads and record alignment info
        for read in sam_file.fetch():
            ...
            if(not read.query_name in seq_dict):
            ...
            tsv_row.extend([cigar_stats_nt[0], cigar_stats_blk[0], cigar_stats_nt[1], cigar_stats_blk[1], cigar_stats_nt[2], cigar_stats_blk[2]])
        ...
        for i in range(1, int(sys.argv[2])+1):
            if i < 10:
                ...
                sam_file = pysam.AlignmentFile(sys.argv[3] + ".part-0" + str(i) + "_megares_mapped.sam", 'r')
            else:
                fastq_file = sys.argv[1] + ".part-" + str(i) + ".fastq"
                sam_file = pysam.AlignmentFile(sys.argv[3] + ".part-" + str(i) + "_megares_mapped.sam", 'r')
    ...
    with open(os.path.basename(sys.argv[1]) + ".tsv", 'w') as out_tsv:
        ...
        #first row of tsv file is number of reads
        tsv_writer.writerow(["Number of reads: ", num_reads])
        #first row of tsv file is number of reads that aligned to megares
        tsv_writer.writerow(["Number of on target reads: ", len(seq_dict)])
        #empty row
        tsv_writer.writerow([])
        #each row of list to tsv file
