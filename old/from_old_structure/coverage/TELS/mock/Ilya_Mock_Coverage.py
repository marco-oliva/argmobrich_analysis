...
# Alignment files
lenghts = ['D', 'E', 'F']
alignments_base_path = '/home/noyes046/moliva/projects/argmobrich/analysis/coverage/ilya/mock'
sam_files = dict()
for organism in reference_info['organisms_list']:
    sam_files[organism] = dict()
    for length in lenghts:
        sam_files[organism][length] = glob.glob(alignments_base_path + '/' +  '*' + length + '*' + organism + '*.sam')
...
for organism in reference_info['organisms_list']:
    ...
    for length in lenghts: 
        ...
        # Compute coverage
        for samfile_path in sam_files[organism][length]:
            print('organisms_coverage[{}][{}]: processing {}'.format(organism, length, samfile_path))
        ...
        organisms_coverage[organism][length] = tmp_array_dict

# Dump coverage arrays to file
with open('Ilya_hits.json', 'w') as out_file:
