...
if __name__ == "__main__":
    ...
    for sam_file_prefix in sam_file_prefixes:
        ...
        pf_sam_file = pysam.AlignmentFile(sam_file_prefix + "_plasmidfinder.sam", 'r')
    ...
    cat4 = "../mobilome/MOCK_D01"
    cat5 = "../mobilome/MOCK_E01"
    cat6 = "../mobilome/MOCK_F01"
    ...
    for aclame_ref in aclame_samples:
        if "proph" in aclame_ref:
            if aclame_ref in prophage_ref_df.index:
                aclame_tuples.append((aclame_ref, str(prophage_ref_df.loc[aclame_ref].iloc[0])))
            #else:
                #print(aclame_ref)
        else:
            if aclame_ref in plasmid_ref_df.index:
                aclame_tuples.append((aclame_ref, str(plasmid_ref_df.loc[aclame_ref].iloc[0])))
            #else:
                #print(aclame_ref)
    ...
    fig = make_subplots(rows=1, cols=6, specs = [[{"colspan":3},None,None,{"colspan":3, "secondary_y":True},None,None]])
    ...
    #fig.update_yaxes(row=1,col=1,tickmode="array", tickvals=[], ticktext=[], tickfont_size=32)
    fig.update_xaxes(row=1,col=1,tickmode="array", tickvals=[0,1,2], ticktext=["bovine_2kb", "bovine_5kb", "bovine_8kb"],tickfont_size=100)
    fig.update_yaxes(row=1,col=1,tickmode="array", tickvals=[], ticktext=[])
    fig.update_xaxes(row=1,col=4,tickmode="array", tickvals=[0,1,2], ticktext=["mock_2kb", "mock_5kb", "mock_8kb"],tickfont_size=100)
    fig.update_yaxes(row=1,col=4,tickmode="array", tickvals=[], ticktext=[], secondary_y=False)
    #print(mgetick_vals)
    #print(mgetick_text)
    ...
    fig.update_yaxes(row=1,col=5,tickmode="array", tickvals=[], ticktext=[], secondary_y=True, tickfont_size=28)

    #fig.update_yaxes(row=1,col=5,tickmode="array", tickvals=[], ticktext=[])
    fig.write_image("mge_heatmap_full.png")
